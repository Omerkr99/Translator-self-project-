...
| Synchronization method | Partially checked live: host-side wall-clock time anchored at `PCSX.hardResetEmulator()`+`PCSX.resumeEmulator()` reproduced byte-identical frames across two independent runs for the first ~23s of boot (BIOS/logos), and matched 17/20 samples overall including most of a later, faster-changing window — see `docs/renderer/MOVIE_TIME_SOURCE_INVESTIGATION.md`. Not yet a proven frame-exact PS1-side source (the mismatches weren't isolated as jitter vs. real non-determinism), but a real, encouraging partial result for external (O5) subtitle sync specifically. |
| Movie-time source | Not found as a PS1-side (in-RAM) signal; host-side wall-clock timing looks promising for O5's purposes specifically, see above |
